...
class WProfileItem(QtWidgets.QWidget):
    def __init__(self,
                 index: int = None,
                 profiles_tab: QtWidgets.QWidget = None,
                 cst: CST = None,
                 tile: Tile = None,
                 parent=None):
        ...
        self.tile = tile if tile else Tile()
        ...
        for le in self.findChildren(QtWidgets.QLineEdit):
            le.editingFinished.connect(self.update_tile)
        for sb in self.findChildren(NoWheelSpinBox) + self.findChildren(NoWheelDoubleSpinBox):
            sb.valueChanged.connect(self.update_tile)

    def update_tile(self):
        for s in self.findChildren(NoWheelDoubleSpinBox) + self.findChildren(NoWheelSpinBox):
            self.cst.__setattr__(s.objectName(), s.value())
        # print(self.cst.__dict__)

    def update_profile(self):
        """Refactored method from parent to self"""
        for box in self.profiles_tab.findChildren(QtWidgets.QGroupBox):
            for f in box.findChildren(QtWidgets.QLineEdit):
                self.cst.__setattr__(f.objectName(), f.text())
            for c in box.findChildren(QtWidgets.QComboBox):
                self.cst.__setattr__(c.objectName(), c.currentIndex())
            spins = box.findChildren(QtWidgets.QDoubleSpinBox) + box.findChildren(QtWidgets.QSpinBox)
            for s in spins:
                self.cst.__setattr__(s.objectName(), s.value())
            for r in box.findChildren(QtWidgets.QRadioButton):
                self.cst.__setattr__(r.objectName(), r.isChecked())

            tile_bullet_weigh = self.findChild(QtWidgets.QLabel, 'tileBulletWeight')
            weight_combo_box_idx = self.profiles_tab.weightComboBox.currentIndex()
            bullet_weight = str(int(round(self.profiles_tab.bulletWeight.value(), 0))) \
                if weight_combo_box_idx == 0 \
                else str(round(self.profiles_tab.bulletWeight.value(), 1))
            self.cst.__setattr__(
                tile_bullet_weigh.objectName(),
                bullet_weight + ('gr' if weight_combo_box_idx == 0 else 'g')
            )
        self.fill_profile()
        self.fill_tile()

    def fill_tile(self):
        for i, (k, v) in enumerate(self.cst.__dict__.items()):
            item = self.findChild(QtWidgets.QLabel, k)
            item.setText(v) if item else None
            item = self.findChild(NoWheelSpinBox, k)
            item.setValue(v) if item else None
            item = self.findChild(NoWheelDoubleSpinBox, k)
            item.setValue(v) if item else None
        self.findChild(QtWidgets.QLabel, 'label_count')\
            .setText(str(self.parent().layout().indexOf(self)+1))

    def fill_profile(self):
        cur_prof_widget = self.profiles_tab.findChild(QtWidgets.QTabWidget)
        for i, (k, v) in enumerate(self.cst.__dict__.items()):
                item = cur_prof_widget.findChild(QtWidgets.QLineEdit, k)
                item.setText(v) if item else None
                item = cur_prof_widget.findChild(QtWidgets.QSpinBox, k)
                item.setValue(v) if item else None
                item = cur_prof_widget.findChild(QtWidgets.QComboBox, k)
                item.setCurrentIndex(v) if item else None
                item = cur_prof_widget.findChild(QtWidgets.QDoubleSpinBox, k)
                item.setValue(v) if item else None
                item = cur_prof_widget.findChild(QtWidgets.QRadioButton, k)
                item.setChecked(v) if item else None
        # print(self.cst.__dict__)
    ...
    def set_current(self):
        ...
        if self.profiles_tab.state.cur_item:
            self.profiles_tab.state.cur_item.setStyleSheet(self.defStyle)
        self.profiles_tab.state.cur_item = self
